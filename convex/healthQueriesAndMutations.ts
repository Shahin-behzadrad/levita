...
import { internalQuery, internalMutation, query, QueryCtx, MutationCtx } from "./_generated/server";
...
type HealthAnalysisWithLabFile = Doc<"healthAnalyses"> & { labFileName: string | null | undefined };
...
export const getHealthAnalysesForUser = query({
  handler: async (ctx: QueryCtx): Promise<HealthAnalysisWithLabFile[]> => {
    ...
    return Promise.all(analyses.map(async (analysis: Doc<"healthAnalyses">): Promise<HealthAnalysisWithLabFile> => {
...
export const getHealthAnalysisById = query({
    ...
    handler: async (ctx: QueryCtx, args: { analysisId: Id<"healthAnalyses"> }): Promise<HealthAnalysisWithLabFile | null> => {
        const userId = await getAuthUserId(ctx);
        if (!userId) {
            return null;
        }
